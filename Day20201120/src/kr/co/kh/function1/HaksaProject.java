...
public class HaksaProject {
	...
	public static void main(String[] args) {
		...
		Statement stmt = null;
		int cnt = 0;
		HaksaFunction hakObj = new HaksaFunction();
		...
		while (true) {
			...
			if (protocol.equals("1")) {// 등록
				try {
					protocol = hakObj.register(protocol, input, stmt, conn, cnt);
				...
				try {
					...
					hakObj.delete(input, stmt, conn, cnt);
				} catch (SQLException e1) {
					e1.printStackTrace();
				} catch (IOException e1) {
					e1.printStackTrace();
